...
void CCSGCServerSystem::PreInitGC()
{
	...
	GetGCClient()->SetServerSteamdatagramPort(sv_steamdatagramtransport_port.GetInt());
}

void CCSGCServerSystem::PostInitGC()
{
	const CSteamID *id = engine->GetGameServerSteamID();
	if (!id)
		return;
	...
	if (id->IsValid())
	{
		Msg("CCSGCServerSystem - adding listener\n");
		GCClientSystem()->GetGCClient()->AddSOCacheListener(this);
	}
	else
	{
		Warning("CCSGCServerSystem - couldn't add listener because Steam wasn't ready\n");
	}
}
...
{
	// RE TODO: ResetConVarsFromFile("cfg/lan_server.cfg");

	if (m_steamID.IsValid())
	{
		GetGCClient()->RemoveSOCacheListener(this);
	}
...
void GCCStrikeWelcomeMessageReceived(CMsgCStrike15Welcome& msg)
{
	// RE TODO: server_valve
}
