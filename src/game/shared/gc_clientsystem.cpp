...
{
	m_bInittedGC = false;
	m_bHasRecievedGCHello = false;
	m_timeGCWelcomeTimestamp_RelativeToCRTime = 0;
	m_timeGCWelcomeTimestamp = 0;
	m_timeLastRecvWelcome = 0.0;
	m_nCurrency = 255;
}
...
// Purpose:
//-----------------------------------------------------------------------------
RTime32 CGCClientSystem::GCTimeFromLocalTime( RTime32 timeLocal )
{
	if ( m_bHasRecievedGCHello )
		timeLocal += m_timeGCWelcomeTimestamp_RelativeToCRTime;
	return timeLocal;
}
...
// Purpose:
//-----------------------------------------------------------------------------
RTime32 CGCClientSystem::GCTimeToLocalTime( RTime32 timeGC )
{
	if ( m_bHasRecievedGCHello )
		timeGC -= m_timeGCWelcomeTimestamp_RelativeToCRTime;
	return timeGC;
}
...
// Purpose:
//-----------------------------------------------------------------------------
GCSDK::CGCClientSharedObjectCache *CGCClientSystem::GetSOCache( const GCSDK::SOID_t &id )
{
	return m_GCClient.FindSOCache( id, false );
}
...
// Purpose:
//-----------------------------------------------------------------------------
GCSDK::CGCClientSharedObjectCache *CGCClientSystem::FindOrAddSOCache( const GCSDK::SOID_t &id )
{
	return m_GCClient.FindSOCache( id, true );
}
...
void CGCClientSystem::SetupGC()
{
	...
	if ( CommandLine()->FindParm("+dev_simulate_gcdown") )
		m_GCClient.SetSimulateGCConnectionFailure( true );
...
void CGCClientSystem::InitGC()
{
	...
	#else
		ISteamClient *pSteamClient = steamgameserverapicontext->SteamClient();
	...
	uint32 unVersion =
#ifdef CLIENT_DLL
		engine->GetClientVersion()
#else
		engine->GetServerVersion()
#endif
		;

	m_bInittedGC = m_GCClient.BInit( unVersion, pSteamClient, hSteamUser, hSteamUser );
	if ( !m_bInittedGC )
	{
		Warning( "CGCClientSystem - failed to init GCClient!\n" );
		Assert( false );
		return;
	}
}
...
void CGCClientSystem::ProcessWelcomeMessage( const CMsgClientWelcome &msg )
{
	m_bHasRecievedGCHello = true;
	m_timeLastRecvWelcome = Plat_FloatTime();
	m_timeGCWelcomeTimestamp = msg.rtime32_gc_welcome_timestamp();
	m_timeGCWelcomeTimestamp_RelativeToCRTime = m_timeGCWelcomeTimestamp - CRTime::RTime32TimeCur();

	if ( msg.has_currency() ) m_nCurrency = msg.currency();
	m_location.CopyFrom( msg.location() );
