...
// Purpose:
//---------------------------------------------------------------------------------------
const char *IEconTool::GetUseCommandLocalizationToken( const IEconItemInterface *pItem, const char* pszDefault ) const
{
	Assert( pItem );
	...
	const char* pszResult = GetUseString();
	if ( !pszResult ) pszResult = pszDefault;
	return pszResult;
}
