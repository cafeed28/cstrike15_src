...
CMsgGCCStrike15_v2_MatchmakingGC2ClientHello g_GC2ClientHello; // RE TODO: move to lobbyscreen
...
void CCSGCClientSystem::SOCacheSubscribed(const GCSDK::SOID_t owner, GCSDK::ESOCacheEvent eEvent)
{
	if (ClientSteamContext().GetLocalPlayerSteamID().ConvertToUint64() == owner.ID())
	{
		m_pSOCache = GCClientSystem()->GetSOCache(owner);
	}
}

void CCSGCClientSystem::SOCacheUnsubscribed(const GCSDK::SOID_t owner, GCSDK::ESOCacheEvent eEvent)
{
	m_pSOCache = NULL;
}

uint64 CCSGCClientSystem::GetTimePlayedConsecutively() const {}
...
void CCSGCClientSystem::UpdateSteamAppDisableUpdate()
{
	...
	{
		flUpdateTime = Plat_FloatTime() + 900;
		nDisableSeconds = 900;
	}
	else if (g_GC2ClientHello.has_account_id())
	{
		flUpdateTime = 0;
		nDisableSeconds = 0;
	}
	...
	static double flLastUpdateTime;
	static int nLastDisableSeconds;

	if (fabs(flUpdateTime - flLastUpdateTime) >= 60 || nDisableSeconds != nLastDisableSeconds)
	{
		flLastUpdateTime = flUpdateTime;
		nLastDisableSeconds = nDisableSeconds;
		
		if (nDisableSeconds)
		{
			Msg("Telling Steam not to update the app for the next %d seconds\n", nDisableSeconds);
		}
		else
		{
			Msg("Telling Steam it is safe to update the app\n");
		}

		if (g_pSteamAppDisableUpdate)
		{
			g_pSteamAppDisableUpdate->SetAppUpdateDisabledSecondsRemaining(nDisableSeconds);
		}
	}
}

void BDebugWelcomeMessage()
{
	static bool bDebugWelcomeMessage = CommandLine()->FindParm( "-debugwelcome" );
	if ( bDebugWelcomeMessage )
	{
		Msg("  this welcome message is OK\n");
	}
}
...
void ReceiveMatchmakingHelloMessage( CMsgGCCStrike15_v2_MatchmakingGC2ClientHello& msg )
{
	...
	if (msg.has_ongoingmatch() && msg.ongoingmatch().has_reservation())
	{
...
GC_REG_JOB(GCSDK::CGCClient, CGCClientWelcomeJob, "CGCClientWelcomeJob", k_EMsgGCClientWelcome);
//GC_REG_JOB(GCSDK::CGCClient, CGCClientJobSteamDatagramTicket, "CGCClientJobSteamDatagramTicket", k_EMsgGCClientWelcome);

// RE TODO: move to lobbyscreen
GC_REG_JOB(GCSDK::CGCClient, ClientJob_EMsgGCCStrike15_v2_MatchmakingGC2ClientHello, "ClientJob_EMsgGCCStrike15_v2_MatchmakingGC2ClientHello", k_EMsgGCCStrike15_v2_MatchmakingGC2ClientHello);
