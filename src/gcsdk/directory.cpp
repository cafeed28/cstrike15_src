...
bool CDirectory::BInit( KeyValues *pKVDirectory )
{
	...
	FOR_EACH_TRUE_SUBKEY( pKVDirectory, pkvProcess )
	{
		...
		{
			AssertMsg1( false, "Duplicate box \"%s\" encountered while parsing the config", pchProcessName );
			return false;
		}
		...
		{
			AssertMsg1( false, "Process %s defined in the config with no process type associated with it", pchProcessName );
			return false;
		}
		...
		FOR_EACH_SUBKEY( pkvProcess, pkvType )
		{
			...
			{
				AssertMsg1( false, "GC defined on box \"%s\" in the config with no type", pchProcessName );
				return false;
			}
			...
			{
				AssertMsg2( false, "GC defined on box \"%s\" with unknown type \"%s\" encountered while parsing the config", pchProcessName, pchGCType );
				return false;
			}
	...
	{
		AssertMsg1( false, "Incorrect number of master GCs in the config. Expected: 1 Actual: %d", GetGCCountForType( nMasterType ) );
		return false;
	}
...
void CDirectory::RegisterGCType( int32 nTypeID, const char *pchName )
{
	...
	AssertMsg1( !bHasElement, "A GC of type %d has already been registered", nTypeID );
	if ( bHasElement )
		return;

	bHasElement = m_dictRegisteredGCNameToType.HasElement( pchName );
	AssertMsg1( !bHasElement, "A GC type with name \"%s\" has already been registered", pchName );
	if ( bHasElement )
		return;
